...
class SART():
    ...
    def executeAlgorythmn(self):
        ...
        with tf.Session(config=config) as sess:
            pdb.set_trace()
            temp_D = tf.ones([self.numtheta, self.numbin])
            temp_M = tf.ones([self.numpix, self.numpix])
            if(self.geom == 'parallel'):
                M = projection_2d.parallel_projection2d(temp_M, self.geometry)
                D = backprojection_2d.parallel_backprojection2d(temp_D, self.geometry)
            else:
                M = projection_2d.fan_projection2d(temp_M, self.geometry)
                D = backprojection_2d.fan_backprojection2d(temp_D, self.geometry)
            

  
            D = tf.maximum(D,eps)
            M = tf.maximum(M*self.dx,eps)
            ...
            for k in range(self.numits):
                ...
                if(self.geom == 'parallel'):
                    bp = backprojection_2d.parallel_backprojection2d(tmp1, self.geometry)
                else:
                    bp = backprojection_2d.fan_backprojection2d(tmp1, self.geometry)
